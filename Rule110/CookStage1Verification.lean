...
theorem cook_c2_tape_bit_ax_partial_with_support_idx_upto7_witness (L slot n idx₀ idx : ℕ)
    (hL : L ≤ 7) (hslot : slot < L) (hn : n < 2^L) :
    cook_c2_decode_at slot
      (infRule110Steps 30
        (cts_to_rule110_tape_with_support_idx (CyclicTagSystem.mk []) idx₀ (natToWord L n))) =
      (natToWord L n).getD slot false :=
  cook_c2_tape_bit_ax_partial_with_support_idx_upto7 L slot n idx₀ idx hL hslot hn

theorem cook_c2_tape_bit_decoder_exists_upto7_witness (slot : ℕ) (hslot : slot ≤ 6) :
      ...
      ∀ (L n idx₀ idx : ℕ), L ≤ 7 → slot < L → n < 2 ^ L →
        decode_bit
            (infRule110Steps 30
              (cts_to_rule110_tape_with_support_idx (CyclicTagSystem.mk []) idx₀ (natToWord L n))) =
          (natToWord L n).getD slot false :=
  cook_c2_tape_bit_decoder_exists_upto7 slot hslot
